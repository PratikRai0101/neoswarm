#!/bin/bash
# One-time Microsoft 365 authentication for NeoSwarm.
# Run this once to cache your M365 token. After that, M365 works in NeoSwarm automatically.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(dirname "$SCRIPT_DIR")"

CACHE_DIR="$HOME/.neoswarm"
mkdir -p "$CACHE_DIR"
...
echo ""
echo "  Microsoft 365 Login for NeoSwarm"
echo "  ─────────────────────────────────"
echo "  A browser window will open for you to sign in."
echo "  After login, the token is cached and M365 works in NeoSwarm automatically."
echo ""
...
if [ -f "$MS365_MCP_TOKEN_CACHE_PATH" ]; then
    ...
    echo "  ✓ M365 is ready to use in NeoSwarm!"
    echo ""
else
    echo ""
