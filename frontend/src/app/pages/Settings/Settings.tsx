...
const API_KEY_STEPS = [
  ...
  },
  {
    ...
    detail: 'Click the "Create Key" button. Name it anything you like (e.g. "NeoSwarm").',
  },
  {
    title: 'Copy your key',
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: updateStatus === 'downloading' ? 1 : 0 }}>
            <Box>
              ...
              <Typography sx={descSx}>
                ...
                {updateStatus === 'idle' && 'Check for new versions of NeoSwarm.'}
              </Typography>
            </Box>
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', pt: 2.5, pb: 1, gap: 2.5, animation: 'fadeIn 0.2s ease', '@keyframes fadeIn': { from: { opacity: 0 }, to: { opacity: 1 } } }}>
          ...
          <Box sx={{ p: 1.5, borderRadius: `${c.radius.md}px`, border: `1px solid ${c.border.subtle}` }}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
              <Box sx={{ width: 8, height: 8, borderRadius: '50%', bgcolor: c.status.success }} />
              <Typography sx={{ fontSize: '0.78rem', fontWeight: 600, color: c.text.primary }}>
                Ollama (local)
              </Typography>
            </Box>
            <Typography sx={{ ...descSx, mb: 0, mt: 0.5 }}>
              No API key is required. NeoSwarm automatically discovers models from Ollama at localhost:11434.
            </Typography>
          </Box>
          ...
          <Typography sx={{ ...descSx, mb: -1 }}>
            Connect a supported cloud provider. Keys are stored only on this device.
          </Typography>
