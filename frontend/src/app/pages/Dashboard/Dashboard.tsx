...
const DashboardInner: React.FC<DashboardProps> = ({ dashboardId, isActive = true }) => {
  ...
  const [showWalkthrough, setShowWalkthrough] = useState(() => {
    if (localStorage.getItem('neoswarm_walkthrough_pending') === 'true') {
      return true;
    }
    return false;
  });
  ...
  const handleWalkthroughComplete = useCallback(() => {
    ...
    localStorage.removeItem('neoswarm_walkthrough_pending');
    localStorage.setItem('neoswarm_walkthrough_seen', 'true');
  }, []);
  ...
  useEffect(() => {
    if (!dashboardId) return;
    (window as any).__neoswarm_last_dashboard_id = dashboardId;
  }, [dashboardId]);
