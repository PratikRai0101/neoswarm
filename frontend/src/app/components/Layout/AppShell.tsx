...
const SIDEBAR_WIDTH_KEY = 'neoswarm-sidebar-width';
const UPDATE_DISMISS_KEY = 'neoswarm-update-dismissed';
...
const AppShell: React.FC = () => {
  ...
  const handleDownloadUpdate = useCallback(async () => {
    try { await (window as any).neoswarm?.downloadUpdate(); } catch {}
  }, []);
  ...
  const openUrlInBrowser = useCallback((url: string, webContentsId?: number) => {
    ...
    if (dashMatch) {
      ...
      const lastId = (window as any).__neoswarm_last_dashboard_id as string | undefined;
  ...
  useEffect(() => {
    ...
    if (!w.neoswarm?.onWebviewNewWindow) return;
    let lastUrl = '';
    let lastTime = 0;
    return w.neoswarm.onWebviewNewWindow((url: string, webContentsId: number) => {
      const now = Date.now();
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100vh', bgcolor: c.bg.page }}>
      ...
      {showUpdateBanner && (
        ...
        >
          ...
          <Typography sx={{ fontSize: '0.8rem', color: c.text.secondary, flex: 1, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
            {updateStatus === 'available' && `NeoSwarm ${availableVersion} is available`}
            {updateStatus === 'downloading' && `Downloading NeoSwarm ${availableVersion}…`}
            {updateStatus === 'downloaded' && `NeoSwarm ${availableVersion} is ready to install`}
          </Typography>
          ...
          <Box sx={{ px: 1, mb: 0.25 }}>
            <ListItemButton
              ...
              sx={{
                ...
                py: 0.6,
                px: 1.25,
                bgcolor: isGitRoute ? `${c.accent.primary}12` : 'transparent',
                '&:hover': { bgcolor: isGitRoute ? `${c.accent.primary}18` : `${c.text.tertiary}0A` },
                transition: 'background-color 0.15s',
              }}
            >
              <ListItemIcon sx={{ color: isGitRoute ? c.accent.primary : c.text.tertiary, minWidth: 32 }}>
      ...
      >
        ...
        >
          {updateStatus === 'available' && `NeoSwarm ${availableVersion} is available`}
          {updateStatus === 'downloaded' && `NeoSwarm ${availableVersion} downloaded — restart to update`}
        </Alert>
      </Snackbar>
    </Box>
  );
